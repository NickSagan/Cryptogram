...
class Encryption: NSObject
{
    ...
    private class func __encryptData(hexStr: String, hexKey:String, hexIV:String) throws -> String
    {
        ...
        let cryptStatus:CCCryptorStatus = cryptData.withUnsafeMutableBytes { (cryptBytes:UnsafeMutableRawBufferPointer) in
            return data.withUnsafeBytes { (dataBytes:UnsafeRawBufferPointer) in
                return iv.withUnsafeBytes { (ivBytes:UnsafeRawBufferPointer) in
                    return key.withUnsafeBytes { (keyBytes:UnsafeRawBufferPointer) in
                        
                        let _cryptBytes = cryptBytes.baseAddress!.assumingMemoryBound(to: Data.self)
                        let _dataBytes = dataBytes.baseAddress!.assumingMemoryBound(to: Data.self)
                        let _ivBytes = ivBytes.baseAddress!.assumingMemoryBound(to: Data.self)
                        let _keyBytes = keyBytes.baseAddress!.assumingMemoryBound(to: Data.self)
                        ...
                        return CCCrypt(CCOperation(kCCEncrypt),
                                ...
                                _keyBytes, keyLength,
                                _ivBytes,
                                _dataBytes, data.count,
                                _cryptBytes, cryptLength,
                                &numBytesEncrypted)
                    }
                }
            }
